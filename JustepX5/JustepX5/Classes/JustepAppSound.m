...
// on 12-6-8.
//
#import "JustepAppSound.h"
#import "JustepViewController.h"

#define DOCUMENTS_SCHEME_PREFIX		@"documents://"
#define HTTP_SCHEME_PREFIX			@"http://"

@implementation JustepAppSound
...
- (NSURL*) urlForResource:(NSString*)resourcePath
{
    ...
    if ([resourcePath hasPrefix:HTTP_SCHEME_PREFIX]){
        ...
        filePath = [resourcePath stringByReplacingOccurrencesOfString:DOCUMENTS_SCHEME_PREFIX withString:[NSString stringWithFormat:@"%@/",[JustepViewController applicationDocumentsDirectory]]];
       NSLog(@"Will use resource '%@' from the documents folder with path = %@", resourcePath, filePath);
    } else {
        // attempt to find file path in www directory
        filePath = [self.appDelegate pathForResource:resourcePath];
        if (filePath != nil) {
...
- (JustepAppAudioFile*) audioFileForResource:(NSString*) resourcePath withId: (NSString*)mediaId
{
	...
	JustepAppAudioFile* audioFile = nil;
	NSURL* resourceURL = nil;
	...
	if (audioFile == nil){
		...
		if (bError) {
			jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"justepApp.Media.onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: errcode message: errMsg]];
			[super execJS:jsString];
		} else {
			audioFile = [[[JustepAppAudioFile alloc] init] autorelease];
...
- (void) play:(NSString *)soundId withSrc:(NSString *)src withDict:(NSMutableDictionary *)options
{
	...
	NSString* mediaId = soundId;
	BOOL bError = NO;
	NSString* jsString = nil;
	
	JustepAppAudioFile * audioFile = [self audioFileForResource:src withId: mediaId];
	...
	if (audioFile != nil) {
		...
		if (!bError){
            ...
            if (!bError) {
                ...
                jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"justepApp.Media.onStatus", mediaId, MEDIA_DURATION, position, @"justepApp.Media.onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                [super execJS:jsString];
        ...
        if (bError) {
			...
			jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"justepApp.Media.onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_NONE_SUPPORTED message: nil]];
			[super execJS:jsString];
		}
	}
...
- (void) prepare:(NSString *)callbackId withId:(NSString *)soundId withSrc:(NSString *)src
{
	...
	JustepAppAudioFile* audioFile = [[self soundCache] objectForKey: mediaId];
    if (audioFile == nil) {
        // did not already exist, try to create
        audioFile = [self audioFileForResource:src withId: mediaId];
        if (audioFile == nil) {
            // create failed
            bError = YES;
        } else {
    ...
    if (!bError) {
        ...
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);\n%@", @"justepApp.Media.onStatus", mediaId, MEDIA_STATE, state, [result onSuccessString:callbackId]];
...
- (void) stop:(NSString *)soundId withSrc:(NSString *)src
{
	...
	if (audioFile != nil && audioFile.player!= nil) {
        ...
        [audioFile.player stop];
        audioFile.player.currentTime = 0;
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"justepApp.Media.onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
...
- (void) pause:(NSString *)soundId withSrc:(NSString *)src
{
	...
	if (audioFile != nil && audioFile.player != nil) {
            ...
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"justepApp.Media.onStatus", mediaId, MEDIA_STATE, MEDIA_PAUSED];
	} 
    // ignore if no media playing
...
- (void) seekTo:(NSString *)soundId withSrc:(NSString *)src toTimeStamp:(NSString *)timeStamp
{
	//args:
	// 0 = callbackId
    // 1 = Media id
	...
	JustepAppAudioFile* audioFile = [[self soundCache] objectForKey: mediaId];
    double position = [timeStamp doubleValue];
	
    if (audioFile != nil && audioFile.player != nil && position){
        double posInSeconds = position/1000;
        audioFile.player.currentTime = posInSeconds;
        NSString* jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%f);", @"justepApp.Media.onStatus", mediaId, MEDIA_POSITION, posInSeconds];
...
- (void) getCurrentPosition:(NSString *)callbackId withId:(NSString *)soundId withSrc:(NSString *)src
{
    ...
    JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageAsDouble: position];
	NSString* jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%.3f);\n%@", @"justepApp.Media.onStatus", mediaId, MEDIA_POSITION, position, [result onSuccessString:callbackId]];
    [super execJS:jsString];
	...
	if (audioFile != nil) {
        ...
        if ([self hasAudioSession]) {
            ...
            if (![self.avSession  setActive: YES error: &error]){
                ...
                jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"justepApp.Media.onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_ABORTED message: errorMsg] ];
                [super execJS:jsString];
                return;
            }
        }
		...
		if (error != nil) {
            ...
            jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"justepApp.Media.onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
		}
	}
    if (jsString) {
       [super execJS:jsString]; 
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
    ...
    if (flag){
        ...
        jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%@);", @"justepApp.Media.onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode: MEDIA_ERR_DECODE message:nil]];
    }
    if (self.avSession) {
        [self.avSession setActive:NO error:nil];
    }
...
@implementation JustepAppAudioFile
