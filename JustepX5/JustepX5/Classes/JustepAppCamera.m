...
#import "JustepAppCamera.h"
#import "NSData+Base64.h"
#import "NSDictionaryExtension.h"
#import <MobileCoreServices/UTCoreTypes.h>

@implementation JustepAppCamera
...
- (void) getPicture:(NSString *)callbackId withDict:(NSMutableDictionary*)options
{
	...
	if (!hasCamera) {
		...
		JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: @"no camera available"];
        ...
        { 
            if ([self.appViewController respondsToSelector:@selector(presentViewController:::)]) {
                [self.appViewController presentViewController:self.pickerController animated:YES completion:nil];
            } else {
                [self.appViewController presentModalViewController:self.pickerController animated:YES ];
            }              
        }
    }
}
    ...
    NSData* data = nil;
        ...
        if (self.pickerController.returnType == DestinationTypeFileUri){
			...
			if (![data writeToFile: filePath options: NSAtomicWrite error: &err]){
				result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: [err localizedDescription]];
				jsString = [result onErrorString:callbackId];
			}else{
				result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: [[NSURL fileURLWithPath: filePath] absoluteString]];
				jsString = [result onSuccessString:callbackId];
			}
			[fileMgr release];
			
		}else{
			result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: [data base64EncodedString]];
			jsString = [result onSuccessString:callbackId];
        ...
        result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: moviePath];
        jsString = [result onSuccessString:callbackId];
...
{	
	...
	JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsString: @"no image selected"]; // error callback expects string ATM

	[self.webView stringByEvaluatingJavaScriptFromString:[result onErrorString: callbackId]];
