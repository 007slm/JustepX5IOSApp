...
static NSDictionary*	JustepAppContactsW3CtoAB = nil;
static NSDictionary*	JustepAppContactsABtoW3C = nil;
static NSSet*			JustepAppContactsW3CtoNull = nil;
static NSDictionary*	JustepAppContactsobjectAndProperties = nil;
static NSDictionary*	JustepAppContactsdefaultFields = nil;

@implementation JustepAppContact : NSObject
...
{
	if (JustepAppContactsABtoW3C == nil) {
		JustepAppContactsABtoW3C = [NSDictionary dictionaryWithObjectsAndKeys: 
		...
		[JustepAppContactsABtoW3C retain];
	}
	
	return JustepAppContactsABtoW3C;
}
+(NSDictionary*) defaultW3CtoAB
{
	if (JustepAppContactsW3CtoAB == nil){
		JustepAppContactsW3CtoAB = [NSDictionary dictionaryWithObjectsAndKeys: 
				...
				nil];
		[JustepAppContactsW3CtoAB retain];
	}
	return JustepAppContactsW3CtoAB;
}
+(NSSet*) defaultW3CtoNull
{
	
	// these are values that have no AddressBook Equivalent OR have not been implemented yet
	if (JustepAppContactsW3CtoNull == nil){
		JustepAppContactsW3CtoNull = [NSSet setWithObjects: kW3ContactDisplayName,
				kW3ContactCategories, kW3ContactFormattedName, nil];
	[JustepAppContactsW3CtoNull retain];
	}
	return JustepAppContactsW3CtoNull;
}
/*  
...
{
	if (JustepAppContactsobjectAndProperties == nil){
		JustepAppContactsobjectAndProperties = [NSDictionary dictionaryWithObjectsAndKeys: 
													 ...
													 nil];
		[JustepAppContactsobjectAndProperties retain];
	}
	return JustepAppContactsobjectAndProperties;
}
+(NSDictionary*) defaultFields
{
	if (JustepAppContactsdefaultFields == nil){
		JustepAppContactsdefaultFields = [NSDictionary dictionaryWithObjectsAndKeys:
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactName], kW3ContactName,
			[NSNull null], kW3ContactNickname,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactAddresses], kW3ContactAddresses,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactOrganizations], kW3ContactOrganizations,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactPhoneNumbers], kW3ContactPhoneNumbers,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactEmails], kW3ContactEmails,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactIms], kW3ContactIms,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactPhotos], kW3ContactPhotos,
			[[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactUrls], kW3ContactUrls,
			[NSNull null],kW3ContactBirthday,
			[NSNull null],kW3ContactNote,
			nil];
		[JustepAppContactsdefaultFields retain];

	}
	return JustepAppContactsdefaultFields;
}
...
+(void) releaseDefaults
{
	// ugly but it works
	if (JustepAppContactsABtoW3C != nil) {
		[JustepAppContactsABtoW3C release];
		if ([JustepAppContactsABtoW3C retainCount] == 1)
			JustepAppContactsABtoW3C = nil;
	}
	if (JustepAppContactsW3CtoAB != nil) {
		[JustepAppContactsW3CtoAB release];
		if ([JustepAppContactsW3CtoAB retainCount] == 1)
			JustepAppContactsW3CtoAB = nil;
	}
	if (JustepAppContactsW3CtoNull != nil) {
		[JustepAppContactsW3CtoNull release];
		if ([JustepAppContactsW3CtoNull retainCount] == 1)
			JustepAppContactsW3CtoNull = nil;
	}if (JustepAppContactsobjectAndProperties != nil) {
		[JustepAppContactsobjectAndProperties release];
		if ([JustepAppContactsobjectAndProperties retainCount] == 1)
			JustepAppContactsobjectAndProperties = nil;
	}
	if (JustepAppContactsdefaultFields != nil) {
		[JustepAppContactsdefaultFields release];
		if ([JustepAppContactsdefaultFields retainCount] == 1)
			JustepAppContactsdefaultFields = nil;
	}
}
...
-(bool) setFromContactDict:(NSMutableDictionary*) aContact asUpdate: (BOOL) bUpdate
{
	...
	if ([dict isKindOfClass:[NSDictionary class]]){
		bName = true;
		NSArray* propArray = [[JustepAppContact defaultObjectAndProperties] objectForKey: kW3ContactName];
		for(id i in propArray){
			if (![(NSString*)i isEqualToString:kW3ContactFormattedName]){  //kW3ContactFormattedName is generated from ABRecordCopyCompositeName() and can't be set
				[self setValue:[dict valueForKey:i] forProperty: (ABPropertyID)[(NSNumber*)[[JustepAppContact defaultW3CtoAB] objectForKey: i]intValue] 
...
-(bool) addToMultiValue: (ABMultiValueRef) multi fromDictionary: dict{
	...
	if (IS_VALID_VALUE(value)){
		NSString* label = (NSString*)[JustepAppContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
	...
	if (!bUpdate){
		...
		if (copy != nil){
			...
			for(NSDictionary* dict in fieldArray){
				...
				label = (NSString*)[JustepAppContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
...
-(NSMutableDictionary*) translateW3Dict: (NSDictionary*) dict forProperty: (ABPropertyID) prop
{
	...
	NSArray* propArray = [[JustepAppContact defaultObjectAndProperties] valueForKey: [[JustepAppContact defaultABtoW3C] objectForKey:[NSNumber numberWithInt: prop]]];
	...
	for(NSString* key in propArray){ // for each W3 Contact key get the value
		if ((value = [dict valueForKey:key]) != nil && ![value isKindOfClass:[NSNull class]]){
			...
			if ([JustepAppContact needsConversion: key]){ // IM types must be converted
				setValue = (NSString*)[JustepAppContact convertContactTypeToPropertyLabel:value];
			...
			[newDict setObject:setValue forKey: (NSString*)[[JustepAppContact defaultW3CtoAB] valueForKey:(NSString*)key]];
		}
	}
	if ([newDict count] ==0){
		...
		if (copy) {
			...
			for (NSDictionary* field in array) {
				...
				if (idx != kCFNotFound){
                    ...
                    {
                        CFStringRef w3cLabel = [JustepAppContact convertContactTypeToPropertyLabel:testLabel];
                    ...
                    for (id k in propArray){
						...
						if (bSet){  
							NSString* setValue = [JustepAppContact needsConversion:(NSString*)k] ? (NSString*)[JustepAppContact convertContactTypeToPropertyLabel:value] : value;
							[dict setObject:setValue forKey: (NSString*)[[JustepAppContact defaultW3CtoAB] valueForKey:(NSString*)k]];
							...
							valueAB = [existingDictionary valueForKey:[[JustepAppContact defaultW3CtoAB] valueForKey:k]];
							if (valueAB != nil){
								[dict setValue:valueAB forKey:[[JustepAppContact defaultW3CtoAB] valueForKey:k]];
							}
...
-(NSDictionary*) toDictionary: (NSDictionary*) withFields
{
	...
	if (self.returnFields == nil){
		// if no returnFields specified, W3C says to return empty contact (but this method will at least return id)
		return nc;
	}
...
-(NSObject*) extractName
{
	...
	for (NSString* i in fields){
		if ([i isEqualToString:kW3ContactFormattedName]){
			...
			value = [(NSString*)ABRecordCopyValue(self.record, (ABPropertyID)[[[JustepAppContact defaultW3CtoAB] valueForKey:i] intValue]) autorelease];
...
-(NSObject*) extractMultiValue: (NSString*)propertyId
{
	...
	NSNumber* propNumber = [[JustepAppContact defaultW3CtoAB] valueForKey:propertyId];
	...
	if (count){
		...
		for (CFIndex i=0; i<count; i++) {
			...
			if ([fields containsObject:kW3ContactFieldType]){
				...
				value = [JustepAppContact convertPropertyLabelToContactType: label];
...
-(NSObject*) extractAddresses 
{
	...
	for (CFIndex i = 0; i < count; i++) {
        ...
        [newAddress setObject: (label != nil) ? (NSObject*) [[JustepAppContact class] convertPropertyLabelToContactType:label] : [NSNull null] forKey:kW3ContactFieldType];
		...
		for(id k in fields){
			bool bFound;
			id key = [[JustepAppContact defaultW3CtoAB] valueForKey:k];
	...
	if (count){
		...
		for (CFIndex i = 0; i < ABMultiValueGetCount(multi); i++) {
			...
			if ([fields containsObject: kW3ContactFieldType]){
				...
				[newDict setObject: (bFound && value != NULL) ? (id)[[JustepAppContact class ]convertPropertyLabelToContactType: value] : [NSNull null] forKey: kW3ContactFieldType];
			}
			// always set ID
			id identifier = [NSNumber numberWithUnsignedInt: ABMultiValueGetIdentifierAtIndex(multi,i)];
...
-(NSObject*) extractOrganizations
{
	...
	for (id i in fields){
		id key = [[JustepAppContact defaultW3CtoAB] valueForKey:i];
...
{	//NSLog(@"getting self.returnFields");
	...
	if (fieldsArray != nil && [fieldsArray isKindOfClass:[NSArray class]]){
        if ([fieldsArray count] == 1 && [[fieldsArray objectAtIndex:0] isEqualToString:@"*"]) {
            return [JustepAppContact defaultFields];  // return all fields
        }
		for (id i in fieldsArray){
			...
			id fields = [[JustepAppContact defaultObjectAndProperties] objectForKey:name];
			...
			if (fields != nil && property == nil){ //request was for full object
				...
				id abEquiv = [[JustepAppContact defaultW3CtoAB] objectForKey:property];
				if (abEquiv || [[JustepAppContact defaultW3CtoNull] containsObject:property]){
				...
				id valid = [[JustepAppContact defaultW3CtoAB] objectForKey:name];
				if (valid || [[JustepAppContact defaultW3CtoNull] containsObject:name]){
...
-(BOOL) testStringValue: (NSString*)testValue forW3CProperty: (NSString*) property {
	...
	if ([[JustepAppContact defaultW3CtoAB] valueForKeyIsNumber: property ]) {
		ABPropertyID propId = [[[JustepAppContact defaultW3CtoAB] objectForKey: property] intValue];
...
-(BOOL) searchContactFields: (NSArray*) fields forMVStringProperty: (ABPropertyID) propId withValue: testValue {
	...
	for (NSString* type in fields){
		...
		if ([type isEqualToString: kW3ContactFieldType]){
			if ([JustepAppContact isValidW3ContactType: testValue]){
				// only search types if the filter string is a valid ContactField.type
				testString = (NSString*)[JustepAppContact convertContactTypeToPropertyLabel:testValue];
			}
		} else {
...
-(BOOL) searchContactFields: (NSArray*) fields forMVDictionaryProperty: (ABPropertyID) propId withValue: (NSString*)testValue
{
	...
	for(id dict in values){
		for(NSString* member in fields){
			NSString* abKey = [[JustepAppContact defaultW3CtoAB] valueForKey:member]; // im and address fields are all strings
