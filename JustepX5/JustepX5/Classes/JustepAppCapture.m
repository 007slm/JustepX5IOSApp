...
@implementation JustepAppImagePicker
...
@implementation JustepAppCapture
@synthesize inUse;

-(id)initWithWebView:(UIWebView *)theWebView
{
	self = (JustepAppCapture*)[super initWithWebView:theWebView];
	if(self)
	{
        self.inUse = NO;
    }
...
- (void) captureAudio:(NSString *)callbackId withDict:(NSMutableDictionary*)options
{
    ...
    if (NSClassFromString(@"AVAudioRecorder") == nil) {
        result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject: CAPTURE_NOT_SUPPORTED];
    }
    else if (self.inUse == YES) {
        result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject: CAPTURE_APPLICATION_BUSY];
    } else {
...
- (void) captureImage:(NSString *)callbackId withDict:(NSMutableDictionary*)options
{
	...
	if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject:CAPTURE_NOT_SUPPORTED];
        [self execJS:[result onErrorString:callbackId]];
        
	} else {
	
        if (pickerController == nil) {
            pickerController = [[JustepAppImagePicker alloc] init];
        }
        ...
        // JustepAppImagePicker specific property
...
-(NSString*) processImage: (UIImage*) image type: (NSString*) mimeType forCallbackId: (NSString*)callbackId 
{
    ...
    if (![data writeToFile: filePath options: NSAtomicWrite error: &err]){
        result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageToErrorObject: CAPTURE_INTERNAL_ERR ];
        ...
        result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsArray: fileArray cast:@"justepApp.capture._castMediaFile"];
...
- (void) getMediaModes: (NSMutableArray*)arguments withDict:(NSMutableDictionary*)options
{
   // NSString* callbackId = [arguments objectAtIndex:0];
    ...
    NSDictionary* modes = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          nil];
    NSString* jsString = [NSString stringWithFormat:@"justepApp.capture.setSupportedModes(%@);", [modes JSONString]];
...
- (void) getFormatData:(NSString *)callbackId fromPath:(NSString *)path withType:(NSString *)type
{
    ...
    if (!mimeType){
        ...
        id command = [self.appViewController getCommandInstance: @"JustepAppFile"];
        bError = !([command isKindOfClass:[JustepAppFile class]]);
        if (!bError) {
            JustepAppFile *appFile = (JustepAppFile *)command;
            mimeType = [appFile getMimeTypeFromPath:fullPath];
            if (!mimeType) {
    ...
    if (!bError) {
        ...
        result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageAsDictionary:formatData];
        jsString = [result onSuccessString:callbackId];
        //NSLog(@"getFormatData: %@", [formatData description]);
    }
    if (bError) {
        result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject:errorCode];
        jsString = [result onErrorString:callbackId];
    }
    if (jsString) {
        [self execJS:jsString];
...
-(NSDictionary*) getMediaDictionaryFromPath: (NSString*) fullPath ofType: (NSString*) type
{
    ...
    if(!type) {
        id command = [self.appViewController getCommandInstance:@"JustepAppFile"];
        if([command isKindOfClass:[JustepAppFile class]]) {
            JustepAppFile *appFile = (JustepAppFile *)command;
            NSString* mimeType = [appFile getMimeTypeFromPath:fullPath];
...
{
    JustepAppImagePicker* cameraPicker = (JustepAppImagePicker*)picker;
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController*)picker
{
    ...
    NSString* jsString = nil;
    JustepAppCommandCallback* result = nil;
    result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageToErrorObject:CAPTURE_NO_MEDIA_FILES];
    jsString = [result onErrorString: callbackId];
...
- (id) initWithCommand:  (JustepAppCapture*) theCommand duration: (NSNumber*) theDuration callbackId: (NSString*) theCallbackId 
{
    if ((self = [super init])) {
...
- (void) dismissAudioView: (id) sender
{
    ...
    if ([self.captureCommand.appViewController.modalViewController respondsToSelector:@selector(presentingViewController)]) {
        [[self.captureCommand.appViewController.modalViewController presentingViewController] dismissModalViewControllerAnimated:YES];
    } else {
        [[self.captureCommand.appViewController.modalViewController parentViewController] dismissModalViewControllerAnimated:YES];
    }        
    
    if (!self.resultString) {
        // return error
        JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject:self.errorCode];
        self.resultString = [result onErrorString:callbackId];
    }
    ...
    UIAccessibilityPostNotification(UIAccessibilityScreenChangedNotification, nil);
    // return result
    [self.captureCommand execJS: resultString ];
}
- (void) updateTime
{
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
    ...
    if (flag) {
        ...
        JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus: JustepAppCommandStatus_OK messageAsArray: fileArray cast:@"justepApp.capture._castMediaFile"];
        self.resultString = [result onSuccessString:callbackId];
    } else {
        JustepAppCommandCallback* result = [JustepAppCommandCallback resultWithStatus:JustepAppCommandStatus_OK messageToErrorObject:CAPTURE_INTERNAL_ERR];
        self.resultString = [result onErrorString:callbackId];
    }
}
